...
void Table::showOrder(Order order) const {
	showHeader();
	std::string res = std::format("*{:^14d}*{:^17s}*{:^10s}*{:^22s}*", order.getID(), order.getLaptop().getModelName(), statusTypeToString(order.getStatus()), order.getAdditionalInfo());
...
void Table::shortShow(std::vector<Order> data) const {
	...
	std::cout << std::format("*{:^14s}*{:^17s}*{:^10s}*{:^22s}*", "...", "...", "...", "...") << std::endl;
	showFooter();
}
